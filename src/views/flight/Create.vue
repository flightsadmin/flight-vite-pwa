<template>
  <div class="card card-body mt-4">
    <form @submit.prevent="onSubmit">
      ...
      <button type="submit" class="btn btn-primary bi bi-plus-lg mt-3 float-end"> Create Flight</button>
    </form>
  </div>
</template>

<script setup>
import { auth, createFlight } from '../../firebase'
import { reactive, onBeforeUnmount } from 'vue'
import { useRouter } from 'vue-router'

    const router = useRouter()
    const form = reactive({ flight: '', date: '', sta:'', std:'', registration:'', origin:'', destination:'', createdAt: new Date() })
